...
window.showEDATab = function (tabName, evt) {
  document.querySelectorAll('.eda-content').forEach(tab => tab.classList.remove('active'));
  document.querySelectorAll('.eda-tab').forEach(btn => btn.classList.remove('active'));

  document.getElementById(tabName).classList.add('active');
  const source = evt?.currentTarget || evt?.target || window.event?.target;
...
function analyzeCategoricalVariables() {
  ...
  setTimeout(() => {
    ...
    categoricalCols.forEach(col => {
      if (rawData[0] && rawData[0][col]) {
        createCategoricalChart(col, 'catChart' + chartIndex2);
        chartIndex2++;
      }
    });
  }, 100);
...
function displayMetrics(accuracy, loss) {
  ...
  parts.push('enabling targeted retention campaigns worth $' + savedRevenue + ' in saved revenue.');
  parts.push('</div>');
...
async function init() {
  try {
    // Try to use WebGL if available; otherwise fallback silently
